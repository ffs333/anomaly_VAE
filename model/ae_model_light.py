...
class AELight(BaseAE):
    ...
    def __init__(self, input_shape,
        ...
        self._in_block = nn.Sequential(
            nn.Conv2d(1, hidden_dims[0], kernel_size=3, padding=1, stride=2, padding_mode='circular'),
        ...
        for dim in hidden_dims:
            modules.append(
                nn.Sequential(
                    nn.Conv2d(dim, dim * 2, stride=2, kernel_size=3, padding=1, padding_mode='circular'),
        ...
        self._enc_out = nn.Sequential(nn.Linear(encoder_out_size, hidden_dims[-1] * 4),
                                      nn.ReLU(),
                                      nn.Dropout(dropout_pb))

        self._fc_mu = nn.Linear(hidden_dims[-1] * 4, 128)
        self._fc_var = nn.Linear(hidden_dims[-1] * 4, 128)

        # Decoder

        self._decoder_input = nn.Sequential(
                                    nn.Linear(128, hidden_dims[-1] * 4),
                                    nn.ReLU(),
                                    nn.Dropout(dropout_pb),
                                    nn.Linear(hidden_dims[-1] * 4, encoder_out_size)
                                        )
        ...
        for dim in hidden_dims:
            modules.append(
                nn.Sequential(
                    nn.ConvTranspose2d(dim * 2, dim, kernel_size=3, padding=1, output_padding=1,
                                       stride=2),
                    nn.BatchNorm2d(dim),
                    nn.ReLU(),
        ...
        self._final_layer = nn.Sequential(
            nn.ConvTranspose2d(hidden_dims[-1], hidden_dims[-1], kernel_size=3, padding=1, output_padding=1, stride=2),
